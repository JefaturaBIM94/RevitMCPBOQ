...
{
    public class RunPreconstruccion2ToolHandler : IToolHandler
    {
        public string ToolName
        {
            get { return "run_preconstruccion_2"; }
        }
        ...
        public ResponseEnvelope Handle(UIApplication uiApp, RequestEnvelope request)
        {
            ...
            var categoryFilter = new StructuralCategoryFilterService();
            var mapper = new StructuralQuantityMapperService();
            ...
            var execDecision = executionPolicy.EvaluateForRun("run_preconstruccion_2", options, preflight);
            ...
            foreach (var element in elements)
            {
                ...
                var tempCategory = NormalizeStructuralCategory(element);
                if (!categoryFilter.IsCorrida2Category(tempCategory))
                    continue;

                candidatosValidos++;

                var snap = snapshotService.BuildSnapshot(
            ...
            var envelope = new ToolEnvelope<object>
            {
                ...
                Preflight = preflight,
                Warnings = warnings,
                UserMessage = "Corrida 2 Estructura ejecutada.",
                Data = new
                {
                    rutina = "Preconstruccion 2 - Estructura",
                    ...
                    diagnostico = new
                    {
                        ...
                        modo = "corrida_2_revit_manual_selection"
                    },
                    resumen = resumen,
            ...
            return OkJson(envelope, "Corrida 2 Estructura ejecutada.");
        }

        private static string NormalizeStructuralCategory(Autodesk.Revit.DB.Element element)
        {
            if (element == null || element.Category == null)
                return "";
            ...
            if (catId == (int)Autodesk.Revit.DB.BuiltInCategory.OST_StructuralColumns)
                return "Structural Columns";

            if (catId == (int)Autodesk.Revit.DB.BuiltInCategory.OST_StructuralFraming)
                return "Structural Framing";

            if (catId == (int)Autodesk.Revit.DB.BuiltInCategory.OST_StructuralFoundation)
                return "Structural Foundations";

            if (catId == (int)Autodesk.Revit.DB.BuiltInCategory.OST_Walls)
                return "Walls";

            if (catId == (int)Autodesk.Revit.DB.BuiltInCategory.OST_Floors)
                return "Floors";

            if (catId == (int)Autodesk.Revit.DB.BuiltInCategory.OST_Roofs)
                return "Roofs";

            string rawName = element.Category.Name ?? "";

            if (string.Equals(rawName, "Pads", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(rawName, "Pedestales", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(rawName, "Zapatas", StringComparison.OrdinalIgnoreCase))
            {
                return "Pads";
            }
